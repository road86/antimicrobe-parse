...
ast_data = pd.read_csv('ast_data.csv')
...
#order alphabe
las = sorted(list(ast_data['pathogen'].unique()))
...
las = sorted(list(ast_data['antibiotic'].unique()))
...
las = sorted(list(ast_data['specimen'].unique()))
