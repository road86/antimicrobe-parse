...
popularrangpur_data = os.path.join('..','input_data', 'unsorted', 'AST data_ready to use', 'Popular Diagnostic, Rangpur_Aug 2022 (1).xlsx')

df_urine = pd.read_excel(popularrangpur_data, sheet_name = 'Urine', skiprows = 1, header = [2])
df_pus = pd.read_excel(popularrangpur_data, sheet_name = 'Pus', skiprows = 0, header = [1])
df_wound = pd.read_excel(popularrangpur_data, sheet_name = 'Wound Swab', skiprows = 0, header = [1])
df_stool = pd.read_excel(popularrangpur_data, sheet_name = 'Stool', skiprows = 0, header = [1])
df_hvs = pd.read_excel(popularrangpur_data, sheet_name = 'High Vaginal Swab', skiprows = 0, header = [1])
df_sputum = pd.read_excel(popularrangpur_data, sheet_name = 'Sputum', skiprows = 0, header = [1])

os.makedirs('check') # will return error if file exists
datacheck = 'check' 
## Checks
df_urine.to_csv(f'{datacheck}/check_urinepop.csv')
df_pus.to_csv(f'{datacheck}/check_puspop.csv')
df_wound.to_csv(f'{datacheck}/check_woundpop.csv')
df_stool.to_csv(f'{datacheck}/check_stoolpop.csv')
df_hvs.to_csv(f'{datacheck}/check_hvspop.csv')
df_sputum.to_csv(f'{datacheck}/check_sputumpop.csv')
...
for iii, rrr in df_hvs.iterrows():
    for abioind5 in range_abio_hvs:
        hvs_df = {
            'pathogen': rrr['Organism'],
            'isolates_number': rrr['Number of isolates'],
...
ast_data['location']=pd.Series('rangpur',index=ast_data.index)
ast_data['provider']=pd.Series('popular diagnostic',index=ast_data.index)

ast_data =  ast_data.dropna(subset=['sensitivity']) #remove many datapoints for antibiotics not tested for each sample


ast_data.to_csv(f'{outputloc}/ast_data_popular_diagnostic_rangpur.csv')